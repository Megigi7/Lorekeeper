...
<div class="relationship-gallery">
    <h1>{{ $relationship->character1->name }} & {{ $relationship->character2->name }}'s Gallery</h1>
    <form action="{{ url('/relationships/'. $relationship->id .'/gallery/store') }}" method="post" enctype="multipart/form-data">
        @csrf
            ...
            <div class="gallery-items">
                @foreach($galleryItems as $item)
                    <div class="gallery-item">
                        ...
                        <form action="{{ route('relationship_gallery.destroy', ['id' => $item->id]) }}" method="POST" class="delete-form">
                            @csrf
                            @method('DELETE')
    ...
    <p><a href="{{ url('relationships/' . $relationship->id) }}">Back to relationship Sheet</a></p>
